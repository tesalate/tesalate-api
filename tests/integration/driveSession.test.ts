...
} from '../fixtures/driveSession.fixture';
...
import { VehicleData, DriveSession } from '../../src/models';
...
describe('DriveSession routes', () => {
  ...
  describe('GET /v1/drive-sessions/:driveSessionId', () => {
    test('should return 200 and the drive session object if data is ok', async () => {
      ...
      const dbVehicleData = await VehicleData.find({ drive_session_id: _id.toHexString() }).lean();
      ...
      expect(res.body).toMatchObject({
        ...
        sessionData: {
          ...
          avgSpeed: {
            value: speedArr.reduce((a: number, b: number) => a + b, 0) / speedArr.length,
