...
import { createSocketClient, setupWebSocket } from '../utils/setupWebSocket';
import { userOne, insertUsers, admin, userTwo } from '../fixtures/user.fixture';
...
describe('WebSocket Server', () => {
  ...
  test('Server rejects connection request without token', async () => {
    const [client1, messages1] = await createSocketClient(config.port, '', 1, 3);
